...
class JobTitleStoreInitializer(Initialize):
    ...
    async def initialize(self):
        ...
        if settings.repository_type == 'postgresql':
            ...
            container.register(JobTitleRepositoryInterface, instance=container.resolve(JobTitlePostgresDatabase))
        if settings.repository_type == 'redis':
            await JobTitleRedisDatabaseInitializer().initialize()
            container.register(JobTitleRepositoryInterface, instance=container.resolve(JobTitleRedisDatabase))
