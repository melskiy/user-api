...
@router.get("/get", status_code=status.HTTP_200_OK, name="Получение пользователя", response_model=User)
async def input_data(methods_service: GetUserService = Depends(), repo: RepositoryInterface = Depends(get_repository), id: str = None):
